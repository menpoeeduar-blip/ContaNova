...
export default function Cartera() {
  const { data: stats, isLoading: statsLoading, refetch: refetchStats } = useGetCarteraStats();
    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                {statsLoading ? (
                  ...
                  <h3 className="text-2xl font-bold text-green-500 mt-1">{formatCurrency(stats?.totalPorCobrar || 0)}</h3>
                )}
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                {statsLoading ? (
                  ...
                  <h3 className="text-2xl font-bold text-destructive mt-1">{formatCurrency(stats?.vencidasPorCobrar || 0)}</h3>
                )}
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                {statsLoading ? (
                  ...
                  <h3 className="text-2xl font-bold text-orange-500 mt-1">{formatCurrency(stats?.totalPorPagar || 0)}</h3>
                )}
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                {statsLoading ? (
                  ...
                  <h3 className="text-2xl font-bold text-destructive mt-1">{formatCurrency(stats?.vencidasPorPagar || 0)}</h3>
                )}
      ...
      <Tabs value={tab} onValueChange={setTab} className="w-full">
        <TabsList className="grid w-[400px] grid-cols-2 bg-muted/50 border border-border">
          <TabsTrigger value="cobrar" className="data-[state=active]:bg-card">
            Por Cobrar (Clientes)
          </TabsTrigger>
          <TabsTrigger value="pagar" className="data-[state=active]:bg-card">
            Por Pagar (Proveedores)
          </TabsTrigger>
        </TabsList>
        <div className="mt-6">
          <TabsContent value="cobrar">
            <CuentasCobrarList onChanged={() => refetchStats()} />
          </TabsContent>
          <TabsContent value="pagar">
            <CuentasPagarList onChanged={() => refetchStats()} />
          </TabsContent>
        </div>
      </Tabs>
    </div>
...
function CuentasCobrarList({ onChanged }: { onChanged: () => void }) {
  const { data, isLoading, refetch } = useListCuentasCobrar({});
...
function CuentasPagarList({ onChanged }: { onChanged: () => void }) {
  const { data, isLoading, refetch } = useListCuentasPagar({});
  const safeData = Array.isArray(data) ? data : [];
  return (
    <Card className="border-border shadow-sm overflow-hidden bg-card">
      <div className="overflow-x-auto">
      ...
      <DialogContent className="sm:max-w-xs bg-card border-border">
        ...
        <div className="space-y-4 pt-4">
          <div className="space-y-2">
            ...
            <Input type="number" value={monto} onChange={(e) => setMonto(Number(e.target.value))} max={saldo} />
...
function RegistrarPagoCompraModal({
  compraId,
  saldo,
  onSuccess,
}: {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xs bg-card border-border">
        ...
        <div className="space-y-4 pt-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">Monto a pagar (COP)</label>
            <Input type="number" value={monto} onChange={(e) => setMonto(Number(e.target.value))} max={saldo} />
            <p className="text-xs text-muted-foreground">Saldo actual: {formatCurrency(saldo)}</p>
          </div>
          <Button className="w-full" onClick={handleGuardar} disabled={updateMutation.isPending}>
            Confirmar Pago
          </Button>
        </div>
      </DialogContent>
